...
\begin{examplebox}
    \subsubsection*{Example 5: Taylor Series of a Polynomial About a Nonzero Center}
    ...
    \textbf{Step 2: Evaluate the derivatives at $x = 3$}
    ...
    \textbf{Step 3: Construct the Taylor series}

    The Taylor series about $x_0 = 3$ is given by
    \[
        f(x) = \sum_{n=0}^{\infty} \frac{f^{(n)}(3)}{n!}(x-3)^n.
    \]

    Since all derivatives of order $4$ and higher are zero, the series terminates
    after the cubic term.

    \[
        \begin{aligned}
            x^3 - 10x^2 + 6
             & = f(3) + f'(3)(x-3) + \frac{f''(3)}{2!}(x-3)^2 + \frac{f'''(3)}{3!}(x-3)^3 \\
